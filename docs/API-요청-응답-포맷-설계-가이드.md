...
```
> 버전업 판단 기준
>  - v1 유지 가능 : 기존 요청/응답을 그대로 쓰던 클라이언트가 수정 없이 계속 동작하면 보통 v1 유지
>  - v2 필요 : 기존 클라이언트가 수정 없이는 정상 동작할 수 없으면 v2로 분리

#### API Prefix 정의
- 공통 API Prefix는 `/api/v1`로 한다.
- 관리자 API Prefix는 `/api/v1/admin`으로 한다.
- 인증 API Prefix는 `/api/v1/auth`로 한다.
- 화면 렌더링용 URI와 API URI는 분리한다.

#### API 버전 정책
- API 메이저 버전은 하위 호환성이 깨지는 경우에만 증가시킨다.
- 응답 필드 추가, 신규 API 추가, optional 파라미터 추가는 기존 버전에서 확장한다.
- 요청/응답 필드명 변경, 타입 변경, 필수값 변경, 의미 변경은 새로운 메이저 버전으로 분리한다.
- 버전 증가 전에는 호환 가능한 방식으로 확장 가능한지 우선 검토한다.
